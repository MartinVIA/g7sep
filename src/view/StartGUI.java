...
/**
 * Main JavaFX application window for Cloverville.
 * This class loads stored data, builds the primary user interface,
 * and provides navigation between residents, trades, tasks,
...
public class StartGUI extends Application {
  ...
  /**
   * Updates the community points progress label and progress bar based on
   * the current green points and goal stored in the model.
   */
   ...
   * The method initializes the model, loads persisted data from files,
   * builds the GUI layout, and displays the main application window.
   *
  ...
  public void start(Stage primaryStage) {
    ...
    resident_edit.setOnAction(e -> {
      ...
      popup.setScene(controller.createScene());
      popup.setTitle("Manage Resident: " + selected.getFirstName() + " " + selected.getLastName());
      popup.setOnHidden(ev -> {
        refreshResidentsTable();
        FileWriter.saveResidentsToBinary(model.getAllResidents(), "residents.bin");
        JSONWriter.saveResidentsToJSON(model.getAllResidents(), "docs/file_operations_residents.json");
    ...
    community_points_edit.setOnAction(e -> {
      ...
      submitButton.setOnAction(ev -> {
        try {
          ...
          String displayReward = (currentReward == null || currentReward.isEmpty())
              ? "next green reward"
              : currentReward;
